...
def generate_end_node_content(node):
    ...
    if node.type == "If":
        node.end_content = print('%s [shape=record,label="%s"];' % (generate_end_id(node), node_str(node))) # generate_end_id(node) + node_surround(node)[0] + "\"" + "End If" + "\"" + node_surround(node)[1]
    if node.type == "FuncDef":
        node.end_content = print('%s [shape=record,label="%s"];' % (generate_end_id(node), node_str(node))) #generate_end_id(node) + node_surround(node)[0] + "\"" + "End Function" + "\"" + node_surround(node)[1]
    if node.type == "While":
        node.end_content = print('%s [shape=record,label="%s"];' % (generate_end_id(node), node_str(node))) #generate_end_id(node) + node_surround(node)[0] + "\"" + "End While" + "\"" + node_surround(node)[1]
    if node.type == "For":
        node.end_content = print('%s [shape=record,label="%s"];' % (generate_end_id(node), node_str(node)))#generate_end_id(node) + node_surround(node)[0] + "\"" + "End For" + "\"" + node_surround(node)[1]
    if node.type == "DoWhile_Do":
...
def print_nodes(tree):
    ...
    if not is_if_branch(tree) and not is_root(tree):
        # print(s)
        print('%s [shape=record,label="%s"]' % (node_id(tree), node_str(tree)))
...
def print_mermaid_graph(tree):
    # print("graph TB")
    # print(r"%% " + "Nodes")
    print('digraph "Dominator tree for \'CGiveUp\' function" {')
    print('label="Dominator tree for \'CGiveUp\' function";')
    print_nodes(tree)
    # print(r"%% " + "Links")
